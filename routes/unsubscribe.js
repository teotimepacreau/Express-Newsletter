...
router.delete('/', async(req,res,next)=>{
  ...
  try{
    ...
    if(!existingSubscriber){
      res.status(400).json({error: "Email don't exists in database"})//on envoie en réponse le status 400 (car erreur côté client), le js unsub front récupère et affiche la notif
